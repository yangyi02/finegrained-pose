...
def draw_statistics(annos):
    ...
    for key, value in annos.iteritems():
        a.append(value['azimuth'])
        e.append(value['elevation'])
        t.append(value['theta'])
        d.append(value['distance'])
        f.append(value['f'])
        u.append(value['u'])
        v.append(value['v'])
    a, e, t = np.array(a), np.array(e), np.array(t)
    d, f, u, v = np.array(d), np.array(f), np.array(u), np.array(v)
    ...
    radii = np.histogram(a, bins=bins, weights=weights)
    radii = radii[0]
    ...
    radii = np.histogram(e, bins=bins, weights=weights)
    radii = radii[0]
    max_radii = np.max(radii)
    width = np.ones(N) * 2 * np.pi / N
    bars = ax.bar(angles, radii, width=width, bottom=0.0)
    ...
    # Draw in-plane rotation statistics
    fig = plt.figure()
    ax = fig.add_axes([0.1, 0.1, 0.8, 0.8], polar=True)
    N = 20
    angles = np.arange(-np.pi, np.pi, 2 * np.pi / N)
    ...
    radii = np.histogram(t, bins=bins, weights=weights)
    radii = radii[0]
    max_radii = np.max(radii)
    width = np.ones(N) * 2 * np.pi / N
    bars = ax.bar(angles, radii, width=width, bottom=0.0)
    ...
    ax.set_title('in-plane rotation')
    ...
    fig, ax = plt.subplots()
    ax.hist(u)
    ax.set_title('principal u')
    fig, ax = plt.subplots()
    ax.hist(v)
    ax.set_title('principal v')
    plt.show()
