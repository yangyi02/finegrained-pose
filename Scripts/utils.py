...
'''
def read_obj(obj_file_name, target_file_name='NOT_SAVE', 
    verbose=False, opt_save=False):
    ...
    num_face = len(faces)
    if verbose:
        print "Finish parsing file, num_vertex=%d, num_face=%d"%(num_vertex,
            num_face)
    ...
    if opt_save:
        sio.savemat(target_file_name, obj_data)
    return obj_data
